...
function installHive()
{
 ...
 hiveInfo=`egrep "hive" $wkdir/frames.conf`
 ...
 if [[ $isInstall = "true" && $hiveNode = $node ]];then
     ...
     if [[ ${#hiveIsExists} -ne 0 ]];then
          ...
          #拷贝Mysql连接驱动
          mysqlDrive=`egrep "^mysql-drive" $wkdir/database.conf | cut -d " " -f2`
          #判断驱动是否存在
          ...
          #为Presto开启Thrift服务
          prestoInstall=`egrep "presto" $wkdir/frames.conf | cut -d " " -f2`
          ...
          # 判断是否安装Tez
          tezInstall=`egrep "tez" $wkdir/frames.conf | cut -d " " -f2`
          if [[ $tezInstall = "true" ]];then
            echo "开始安装Tez服务"
            bash ./hadoop/tezcode.sh
          fi
