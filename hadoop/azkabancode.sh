...
function getKeyStore()
{
  ...
  # 注意生成keystore的过程是英文版还是中文版,这里操作系统是最小化安装英文版
  expect << EOF
      spawn /usr/lib/java/jdk1.8.0_144/bin/keytool -keystore keystore -alias jetty -genkey -keyalg RSA
        expect {
                "*password*" { send "$pass_word\n";exp_continue } 
                "*Re-enter*" { send "$pass_word\n";exp_continue }
                "*last name*" { send "brinnatt\n";exp_continue }
                "*unit*" { send "architect\n";exp_continue }
                "*organization*" { send "Greatwall\n";exp_continue }
                "*City*" { send "Capital\n";exp_continue }
                "*Province*" { send "Independency\n";exp_continue }
                "*country*" { send "CN\n";exp_continue }
                "*correct*" { send "y\n";exp_continue }
                "*password*" { send "\n";exp_continue }
                eof
...
{
 # 在frames.conf中查看是否需要安装azkaban
 executorInfo=`egrep "azkaban-executor" $wkdir/frames.conf`
 sqlInfo=`egrep "azkaban-sql" $wkdir/frames.conf`
 webInfo=`egrep "azkaban-web" $wkdir/frames.conf`

 # 获取Mysql信息 
 mysqlNode=`egrep "^mysql-rpm-pack" $wkdir/frames.conf | cut -d " " -f3`
 mysqlUser=`egrep "^azkaban-mysql-user" $wkdir/database.conf | cut -d " " -f2`
 mysqlPasswd=`egrep "^azkaban-mysql-password" $wkdir/database.conf | cut -d " " -f2`
 ...
 if [[ $isExecutorInstall = "true" && $isWebInstall = "true" ]];then
     ...
     if [[ ${#executorIsExists} -ne 0 && ${#webIsExists} -ne 0 ]];then
          ...
          # 生成秘钥
          azkaban_keystore=`egrep "^azkaban-keystore-password" $wkdir/database.conf | cut -d " " -f2`
     ...
     echo "azkaban不被允许安装在 `hostname` 节点"
 fi
}
